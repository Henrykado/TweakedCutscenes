...
void ev0100_s_ecafterlanding(int state)
{
	...
	case 1:
		...
		case Languages_Japanese: // Japanese
			EV_SetFace(player, "IAAAACDEDCD");
			break;
		case Languages_English: // English
		...
		EV_SetFace(tails, "BB");
		EV_Wait(20);
		EV_SetAction(player, &action_s_s0005_sonic, &SONIC_TEXLIST, 0.3f, 0, 20);
		EV_SetAction(player, SONIC_ACTIONS[1], &SONIC_TEXLIST, 1.0f, 1, 5);
		EV_Wait(25);
		EV_ClrFace(player);
		if (VoiceLanguage == Languages_Japanese)
			EV_SetFace(player, "ZZZZ");
		EV_Wait(10);
		EV_MsgCls();
		EV_SerifWait();
		EV_Wait(10);
