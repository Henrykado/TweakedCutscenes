...
void ev0104_s_afterskydeck(int state)
{
	...
	case 1:
		...
		EV_PadOff();
		EV_CanselOn();
		EV_CreatePlayer(2, MilesTalesPrower, -151.22f, 225.0f, -173.36f, 0, 0x4000, 0);
		EV_Wait(1);
		tails = EV_GetPlayer(2);
		EV_SetPos(tails, -151.22f, 225.0f, -173.36f);
		EV_SetAng(tails, 0, 0x4000, 0);
		//TALES_STEP = CSkyWalk_create2(tails, 140.8f); //Used to delay Tails's fall.
