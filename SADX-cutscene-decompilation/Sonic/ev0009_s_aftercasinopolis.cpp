...
void ev0009_s_aftercasinopolis(int state)
{
	...
	case 1:
		...
		cap1 = CreateCaptureBeam(-550.20001f, 0.0f, 978.15002f, 0, 0, 0);
		EV_Wait(1);
		...
		EV_CameraAng(0, 18, 0xF50E, 0x25F8, 0xFF00);
		EV_CameraPos(0, 18, -543.84998f, 5.5999999f, 970.59998f);
		EV_ClrAction(tails);
		...
		EV_SetAction(player, SONIC_ACTIONS[123], &SONIC_TEXLIST, 1.0f, 1, 8);
		EV_SetAction(tails, &action_m_m0001_miles, &MILES_TEXLIST, 1.0f, 1, 8);
		EV_LookObject(player, emerald, 0.0, 1.0, 0.0);
		EV_LookObject(tails, emerald, 0.0, 1.0, 0.0);
		EV_SetAng(player, 0, 0xB6C0, 0);
		EV_SetAng(tails, 0, 0xCDF8, 0);
		...
		EV_SetAction(eggmoble, &action_gm_gm0025_eggmoble, &EV_EGGMOBLE0_TEXLIST, 1.2f, 0, 0);
		...
		EV_Wait(20);
		eggmoble_move_normal(-550.20001f, 11.0f, 978.15002f, 14);
		EV_MsgClose();
