...
void ev0012_s_meetingamy(int state)
{
	...
	case 1:
		...
		EV_SerifWait();
		EV_ClrAction(player);
		EV_SetAction(player, &action_s_s0037_sonic, &SONIC_TEXLIST, 0.5f, 0, 4); // 0 - no loop, walk backwards
		EV_ClrFace(player);
		...
		EV_SetAction(player, &action_s_s0022_sonic, &SONIC_TEXLIST, 1.0f, 1, 4); // 1 - loop, hesitant
		//EV_WaitAction(amy);
