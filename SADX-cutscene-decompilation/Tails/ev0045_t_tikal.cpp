...
void ev0045_t_tikal(int state)
{
	...
	case 1:
		...
		EV_SerifPlay(732);
		EV_Msg(msgTbl_ev0045[TextLanguage][0]); //"\tThe servers are \nthe 7 Chaos Emerald"...
		EV_Wait(90);
		EV_ClrAction(player);
		EV_MsgClose();
		...
		EV_SetFace(tikal, "GD");
		EV_SerifPlay(736);
		EV_Msg(msgTbl_ev0045[TextLanguage][4]); //"\aIt's something \nmy grandmother taugh"...
		EV_SerifWait();
		EV_ClrFace(tikal);
		EV_Wait(10);
		EV_SetFace(tikal, "GAD");
		EV_SerifPlay(737);
		...
		EV_SetFace(tikal, "E");
		EV_SerifPlay(739);
		EV_Msg(msgTbl_ev0045[TextLanguage][7]); //"\t'Chaos is power... \n\tPower enriched"...
