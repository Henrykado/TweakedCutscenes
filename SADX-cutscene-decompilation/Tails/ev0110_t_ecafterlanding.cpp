...
void ev0110_t_ecafterlanding(int state)
{
	...
	case 1:
		...
		case 0: // Japanese
			EV_SetFace(sonic, "IAAAACDEDCD");
			break;
		case 1: // English
			EV_SetFace(sonic, "ICE");
			break;
		}
		...
		EV_Wait(40);
		EV_SetAction(sonic, &action_s_s0005_sonic, &SONIC_TEXLIST, 0.3f, 0, 20);
		EV_SetAction(sonic, SONIC_ACTIONS[1], &SONIC_TEXLIST, 1.0f, 1, 5);
		EV_Wait(25);
		EV_ClrFace(sonic);
		if (VoiceLanguage == Languages_Japanese)
			EV_SetFace(sonic, "ZZZZ");
		EV_Wait(10);
		EV_MsgCls();
		EV_SerifWait();
		EV_Wait(10);
