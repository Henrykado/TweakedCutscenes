...
void ev0033_t_egghornet(int state)
{
	...
	case 1:
		...
		EV_SerifPlay(639);
		EV_Msg(msgTbl_ev0033[TextLanguage][1]); //"\aIt's Eggman!"
		EV_SerifWait();
		EV_Wait(5);
		EV_ClrFace(player);
		...
		EV_MsgW(75, msgTbl_ev0033[TextLanguage][7]); //"\abut now it's time to \nput it to work"...
		EV_MsgClose();
		...
		EV_SerifPlay(645);
		EV_MsgW(75, msgTbl_ev0033[TextLanguage][9]); //"\aDon't even try to \ninterfere this ti"...
		EventSe_Volume(1, -30, 120);
		EV_SerifPlay(646);
		EV_MsgW(80, msgTbl_ev0033[TextLanguage][10]); //"\aGive me that Chaos Emerald,\nor else!"
		EV_SerifWait();
		EV_Wait(10);
