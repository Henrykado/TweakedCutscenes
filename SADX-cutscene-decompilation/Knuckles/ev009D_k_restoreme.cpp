...
void ev009D_k_restoreme(int state)
{
	...
	case 1:
		...
		EV_CameraTargetObj(1, 40, player, 0.0f, 6.0f, 0.0f, 0);
		EV_MovePoint2(player, -2.8699999f, -157.0f, 2201.8999f, 0.34999999f, 0.059999999f);
		EV_Wait(40);
		...
		EV_SetPos(player, -2.8699999f, -157.0f, 2204.8999f);
		EV_SetAng(player, 0, 0x8000, 0);
		...
		EV_Wait(20);
		crushLightOff();
		EV_SetPos(player, -2.45f, -157.0f, 2192.3999f);
		EV_SetAng(player, 0, 0x8000, 0);
