...
void ev0088_k_eggmantricksknuckles(int state)
{
	...
	case 1:
		...
		EV_ClrFace(eggman);
		EV_MsgClose();
		EV_CameraPos(1, 0, 13.7f, 12.0f, 68.25f);
		...
		EV_MsgW(90, msgTbl_ev0088[TextLanguage][1]); //"\aYou have yet to see \nthe true powers"...
		EV_ClrFace(eggman);
		//EV_MsgClose();
		EV_Wait(20);
		EV_ClrAction(player);
		...
		EV_SerifPlay(1052);
		EV_Msg(msgTbl_ev0088[TextLanguage][3]); //"\aI know something that might."
		EV_SerifWait();
		EV_Wait(15);
		EV_ClrFace(eggman);
		EV_MsgClose();
		...
		EV_SerifPlay(1056);
		EV_MsgW(100, msgTbl_ev0088[TextLanguage][7]); //"\aHe's after the pieces of \nthe Master"...
		EV_ClrFace(eggman);
		...
		EV_MovePoint2(player, 8.0f, 0.0f, 34.0f, 0.5f, 1.0f);
		EV_Wait(20);
		EV_MovePoint2(player, 8.0f, 0.0f, 34.0f, 0.3f, 0.30000001f);
		EV_MsgClose();
		EV_Wait(8);
