...
void ev0063_a_twinklepark(int state)
{
	...
	case 1:
		...
		EV_SetPos(player, 362.39999f, 0.0f, 1575.95f);
		EV_SetAng(player, 0, 17740, 0);
		EV_LookPoint(player, 374.0f, 12.0f, 1572.0f);
		...
		EV_ClrAction(sonic);
		EV_SetAction(sonic, &action_s_s0017_sonic, &SONIC_TEXLIST, 0.2f, 0, 8);
		EV_SetAction(sonic, SONIC_ACTIONS[1], &SONIC_TEXLIST, 1.0f, 1, 0);
		EV_SerifWait();
		EV_Wait(5);
		EV_ClrAction(sonic);
		EV_SetAction(sonic, &action_s_s0013_sonic, &SONIC_TEXLIST, 0.25f, 0, 0);
		EV_SetAction(sonic, SONIC_ACTIONS[1], &SONIC_TEXLIST, 1.0f, 1, 0);
		EV_ClrFace(sonic);
		EV_SetFace(sonic, "BCCCYYY");
		EV_SerifPlay(878);
