...
void ev006D_a_savebirdsbros(int state)
{
	...
	case 1:
		...
		EV_MovePoint2(player, 115.86f, 0.0053099999f, 1522.62f, 0.25f, 0.1f);
		stopObject(KOTORI);
		...
		EV_SetAction(KOTORI, &action_w_w0833_wing, &VER1_WING_TEXLIST, 1.0f, 1, 0);
		EV_SetAction(player, &action_a_a0833_amy, &AMY_TEXLIST, 1.0f, 1, 0);
		EV_MovePoint2(player, 115.86f, 0.0053099999f, 1522.62f, 0.16f, 2.0f);
		stopObject(KOTORI);
		...
		//EV_LookObject(player, KOTORI, -1.8f, 0.0f, 0.1f);
		EV_SerifWait();
		EV_Wait(10);
		EV_MsgClose();
		stopObject(KOTORI);
		...
		EV_LookPoint(player, player->twp->pos.x + 3.0f, player->twp->pos.y - 1.0f, player->twp->pos.z + 5.0f);
		EV_SetFace(player, "CD");
		EV_SerifPlay(972);
