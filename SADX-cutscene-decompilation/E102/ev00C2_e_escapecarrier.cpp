...
//		obj_amy = 0; //Needs to use DataPointer for now, because the functions related to task_gattai need to be rewritten to make this way work.
		...
		EV_LookObject(amy, sonic, 0.0f, 4.0f, 0.0f);
		EV_SerifPlay(1289);
		EV_MsgW(50, msgTbl_ev00C2[TextLanguage][8]); //"\aWhat will you do?"
		EV_ClrFace(sonic);
		EV_ClrFace(amy);
		EV_LookFree(sonic);
		EV_LookFree(amy);
		EV_LookObject(sonic, amy, 0.0f, 5.0f, 0.0f);
		...
		EV_SerifPlay(1300);
		EV_Msg(msgTbl_ev00C2[TextLanguage][14]); //"\aWhy do you help me?"
		EV_SerifWait();
		EV_MsgClose();
		EV_Wait(15);
		...
		EV_MovePoint2(tails, -2.0754001f, 1530.46f, 3358.678f, 1.4f, 0.0f);
		EV_LookFree(tails);
		EV_WaitMove(tails);
